...
function delete$(dict3, key2) {
  return map_remove(key2, dict3);
}
function upsert(dict3, key2, fun) {
  let _pipe = dict3;
  let _pipe$1 = map_get(_pipe, key2);
  let _pipe$2 = from_result(_pipe$1);
  let _pipe$3 = fun(_pipe$2);
  return ((_capture) => {
    return insert(dict3, key2, _capture);
  })(_pipe$3);
}
...
function append(first4, second) {
  return append_loop(reverse(first4), second);
}
...
function at_least_decode_tuple_error(size2, data) {
  ...
  let error = (() => {
    ...
    return new DecodeError(_pipe$2, classify_dynamic(data), toList([]));
  })();
      ...
      (list4) => {
        ...
        if ($) {
          ...
          let _pipe$3 = replace_error(
            ...
            toList([
              new DecodeError(
                "A list with at least" + to_string(idx + 1) + "elements",
                "A list with" + to_string(length(list4)) + "elements",
                toList([to_string(idx)])
              )
            ])
          );
...
var ExternalSignal = class extends CustomType {
  constructor(signal) {
    super();
    this.signal = signal;
  }
...
function update(model, msg) {
  if (msg instanceof ServerSavedList) {
    ...
    return [
      (() => {
        if (quantity === 0) {
          return delete$(model, item);
        } else {
          let v = quantity;
          return insert(model, item, v);
        }
      })(),
      none()
    ];
    ...
    return [new_map(), batch(toList([signal]))];
  }
}
function view_new_item() {
  let handle_click = (event2) => {
    ...
    let _pipe = event2;
    let _pipe$1 = at(path, decode_string)(_pipe);
    return map2(
      _pipe$1,
      (var0) => {
...
function view_grocery_item(name, quantity) {
  let handle_input = (e) => {
    ...
    let _pipe$2 = then$(_pipe$1, parse_int);
    let _pipe$3 = map2(
      _pipe$2,
      (_capture) => {
...
function view(model) {
  ...
  let handle_click = (_) => {
    return new Ok(new ExternalSignal(none()));
  };
  return div(
    toList([style(styles)]),
    toList([
